...
"========= MINWOO ========="
"colorscheme desert
set guifont=Bitstream_Vera_Sans_Mono:h11:cDEFAULT

map <F2> :w<cr>:!javac %<cr>
map <F3> :w<cr>:!javac % && java %<cr>
map <F4> :w<cr>:!javac % && java % < %<.in<cr>


map <F5> :w<cr>:!g++ -O2 -DLOCAL % -o %<.exe<cr>
map <F6> :w<cr>:!g++ -O2 -DLOCAL % -o %<.exe && %<.exe<cr>
map <F7> :w<cr>:!g++ -O2 -DLOCAL % -o %<.exe && %<.exe < %<.in <cr>


map <F4> :<CR>:!%<.exe<CR>
map <F9> :so $VIMRUNTIME/syntax/2html.vim<CR>

" 터미널에서의 vim처럼 쓸 수 있음
" behave mswin 옵션은 아래의 mswin.vim 파일 로딩과 함께 사용해야 함
"behave xterm 
" 윈도우의 Ctrl+(C,X,V), 마우스 오른쪽 버튼등을 사용하기 위한 로딩 
" 입력모드에서도, : 명령모드에서도 붙여넣기 됨 
" behave xterm과 함께 사용시 Ctrl+v 가 열블럭 지정이 아닌 붙여넣기로 동작
